...
export default function PracticeView({
  ...
  return (
    <div className={`max-w-3xl mx-auto rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-300 relative ${adhdMode
      ...
      {/* Header */}
      <div className={`p-4 border-b flex justify-between items-center ${loading ? 'pointer-events-none opacity-50' : ''} ${adhdMode ? 'bg-yellow-50 border-yellow-200' : 'bg-slate-50 border-slate-200'
        }`}>
        <button
        ...
        {loading ? (
          ...
          <div className="flex items-center gap-2">
            <span className={`text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 border ${currentQuestion?.source === 'ai_next_api' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-indigo-100 text-indigo-800 border-indigo-200'
              }`}>
            ...
            {adhdMode && <span className="bg-yellow-100 text-yellow-700 text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1"><Accessibility size={12} /> 學習輔助模式</span>}
          </div>
        )}
      </div>
            ...
            <div className="mb-8 relative">
              <div className="flex flex-wrap justify-end gap-3 mb-4">
                {adhdMode && isSpeechSynthesisSupported() && (
                  <div className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-bold ${adhdMode ? 'border-yellow-300 bg-yellow-50 text-yellow-900' : 'border-slate-200 bg-white text-slate-700'
                    }`}>
                    <span>讀題語音</span>
                    <select
                    ...
                    <button
                      ...
                      disabled={loading}
                      className={`flex items-center gap-1 px-2 py-1 rounded-md transition ${adhdMode ? 'bg-yellow-400 hover:bg-yellow-500 text-yellow-900' : 'bg-indigo-600 hover:bg-indigo-700 text-white'
                        }`}
                    >
                      <Volume2 size={14} /> 試聽
                    </button>
              ...
              <div className="text-center">
                {(currentQuestion.imageUrl || currentQuestion.image) && (
                  <div className="mb-6">
                    <div className={`relative mx-auto overflow-hidden rounded-xl border border-slate-200 bg-white ${imageExpanded ? 'max-h-none' : 'max-h-80'
                      }`}>
                      <img
              ...
              {!feedback ? (
                <>
                  {currentQuestion.options ? (
                    <div className={`grid gap-3 w-full ${currentQuestion.options.length === 8 ? 'grid-cols-4' : 'grid-cols-2'}`}>
                      {currentQuestion.options.map((opt, i) => (
                        <button
                          ...
                          disabled={loading}
                          className={`py-4 px-2 rounded-xl font-bold border-2 transition-all shadow-sm text-sm disabled:opacity-50 disabled:cursor-not-allowed break-words overflow-wrap-anywhere ${adhdMode
                              ? (userAnswer === opt
                    ...
                    <div className="relative w-full">
                      <input
                        ...
                        autoFocus
                        disabled={loading}
                        className={`w-full text-center text-2xl p-4 border-2 rounded-xl outline-none transition shadow-inner disabled:opacity-50 disabled:cursor-not-allowed ${adhdMode
                  ...
                  <button
                    ...
                    className={`w-full text-white font-bold py-4 px-6 rounded-xl shadow-lg transition transform active:scale-95 mt-2 disabled:cursor-not-allowed ${adhdMode
                ...
                <div className={`w-full p-6 rounded-xl text-center animate-in zoom-in-95 ${feedback === 'correct'
                  ...
                  {feedback === 'correct' ? (
                    <div className="space-y-4">
                      ...
                      <button
                        ...
                        disabled={loading}
                        className={`mt-4 text-white px-8 py-3 rounded-xl font-bold shadow-lg transition w-full flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${adhdMode
                    ...
                    <div className="space-y-3">
                      {!showExplanation ? (
                        <>
                          ...
                          <div className={`p-4 rounded-xl border text-left mt-2 relative overflow-hidden shadow-sm ${adhdMode
                              ? 'bg-yellow-50 border-2 border-yellow-300'
                              : 'bg-white border border-red-100'
                            }`}>
                            <div className="flex items-center justify-between mb-2">
                              <p className={`text-xs font-bold uppercase flex items-center gap-1 ${adhdMode ? 'text-yellow-900' : 'text-slate-500'
                                }`}>
                        ...
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                          ...
                          <div className={`p-5 rounded-xl border text-left whitespace-pre-wrap leading-relaxed shadow-sm ${adhdMode
                          ...
                          <div className="mt-4 pt-4 border-t border-red-100">
                            ...
                            <button
                              onClick={() => !loading && handleNext()}
                              disabled={loading}
                              className={`w-full text-white px-6 py-3 rounded-xl font-bold shadow transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed ${adhdMode
