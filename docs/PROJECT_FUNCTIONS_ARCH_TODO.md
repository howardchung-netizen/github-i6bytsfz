...
- **後台報表 (Phase 1)**：完成「開發者後台總覽」，包含 `visit_logs` 紀錄寫入、`users` 註冊平台統計擴充、並建立 `/api/metrics` (含 Web/Tablet 區分、DAU/WAU/MAU、訂閱人數等)，與前端 `AnalyticsView` KPI 卡片完美接合。
...
- **PracticeView**：練習/考題流程核心（作答、正誤判斷、學習輔助高亮、語音/翻譯輔助、圖形題圖像放大）
  - 學習輔助語音：退出練習會停止播放、移除語音清單 debug UI
...
- **後台數據檢視與報表（整理版清單）**：
  - **開發者後台總覽** (已完成)
    - (已完成) 下載率（App/Web 來源拆分）
    - (已完成) 每月訂閱人數（新訂、續訂、取消、流失）
    - (已完成) 新帳號申請（依角色：學生/家長/老師/機構）
    - (已完成) 活躍使用者（DAU/WAU/MAU）
    - (已完成) 各帳號登入數量（角色/地區/平台）
    - (已完成) 試題生成量（每日/每科/失敗率）
    - (已完成) 題庫新增量（種子題/圖像題/文字題）
  - **老師後台（班級與作業）**
    - 班級總覽（人數、活躍率）
    - 作業發佈/完成率
    - 學生平均正確率、用時
...
- **語言 / 語音選擇**：
  - 介面中/英切換 (已完成)
  - 數學科可獨立中/英文顯示
  - 學習輔助語音：中文科可選粵/普，英文科只英語，數學依語言選擇
- **翻譯輔助**：中文科英譯中、英文科中譯英
- **數學圖形題後台排版**：支援有圖表/地圖題目的版面向下延展
- **學習輔助報告/學習數據**：產出學習輔助報告，並可結合 ADHD 模式使用率，動態由 AI 醫生提供臨床觀察評估 (已完成)
- **訂製學習菜單 (Premium)**：根據 AI 學習報告結論，自動生成「專屬七日訂製訓練營」(包含中英數跨科混合練習)。(已完成)
- **練習模式 / 考題模式**：已完成，持續驗證
- **收費與訂價**：待核心功能完成後再做（P4）
- **大量上線壓力測試**：待做（P4）
- **申請資助**：待做（P4）
